...
class babylonServerShutdown extends Thread {
...
public class babylonServer extends Thread {
	...
	public babylonServer(String[] args) {
		...
		strings = new babylonStringManager(myURL, Locale.getDefault()
				.getLanguage());
		...
		try {
			...
			// Oops, no log file
			serverOutput(strings.get(thisClass, "noopen") + " " + serverLogName
					+ "\n");
		}
		...
		if (graphics) {
			window = new babylonServerWindow(this, "");
			window.setSize(400, 400);
			window.setVisible(true);
		}

		else {
			System.out.println("\n" + strings.get(thisClass, "serverstatus"));
		...
		try {
			...
			System.out.println("\n" + strings.get(thisClass, "noserversocket"));
			System.exit(1);
		}
		...
		if (javaVersion >= 1.3)
			Runtime.getRuntime().addShutdownHook(
					new babylonServerShutdown(this));
		...
		try {
			...
			while (true) {
				...
				try {
					...
					throw new Exception(strings.get(thisClass, "nosuchuser"));
				}
	...
	protected babylonChatRoom findChatRoom(String roomName) {
		...
		if (mainRoom.name.equals(roomName))
			...
			for (int count = 0; count < chatRooms.size(); count++) {
				tempRoom = (babylonChatRoom) chatRooms.elementAt(count);
	...
	public boolean isUserAllowed(babylonChatRoom room,
		...
		if (room.priv) {
			...
			if (!room.password.equals(password)) {
				...
				if (!invited) {
					try {
						client.sendServerMessage(client.strings.get(thisClass,
								"notallowed")
								+ " " + room.name);
					} catch (IOException e) {
	...
	public synchronized void disconnect(babylonClientSocket who, boolean notify) {
		int count;
		...
		if (notify) {
			try {
				...
				who.sendDisconnect(who.user.id, strings.get(thisClass,
						"disconnected"));
			} catch (IOException e) {
			}
		}
		...
		synchronized (connections) {
			...
			for (count = 0; count < currentConnections; count++) {
				babylonClientSocket other = (babylonClientSocket) connections
						.elementAt(count);
				try {
		...
		if (graphics) {
			synchronized (window.userList) {
				...
				// one disconnection.
				for (count = 0; count < window.userList.getItemCount(); count++) {
					if (window.userList.getItem(count).equals(who.user.name)) {
	...
	public synchronized void disconnectAll(boolean notify) {
		...
		synchronized (connections) {
			...
			for (count = (currentConnections - 1); count >= 0; count--) {
				babylonClientSocket temp = (babylonClientSocket) connections
						.elementAt(count);
		...
		while (!stop) {
			...
			synchronized (connections) {
				for (int count = 0; count < connections.size(); count++) {
					babylonClientSocket tmpClient = (babylonClientSocket) connections
							.elementAt(count);
	...
	protected void acceptReconnect(babylonClientSocket client) {
		...
		try {
			client.istream = new DataInputStream(client.mySocket
					.getInputStream());
			client.ostream = new DataOutputStream(client.mySocket
					.getOutputStream());
			client.interrupt();
		} catch (IOException a) {
		}
...
/*to print the datas to server*/
 
	protected void serverOutput(String message) {
		if (graphics && (window.logWindow != null))
		...
		if (log != null) {
			try {
				...
				if (graphics && (window.logWindow != null))
					window.logWindow.append(strings.get(thisClass,
							"writelogerror")
							+ "\n");
				else
					System.out.print(strings.get(thisClass, "writelogerror")
							+ "\n");
			}
		}
	...
	protected void saveMessages() {
		...
		try {
			...
			for (int count = 0; count < messages.size(); count++) {
				babylonMessage tempMessage = (babylonMessage) messages
						.elementAt(count);
	...
	protected void shutdown() {
		...
		if (currentConnections > 0) {
			...
			synchronized (connections) {
				...
				for (int count = 0; count < currentConnections; count++) {
					babylonClientSocket who = (babylonClientSocket) connections
							.elementAt(count);

					try {
						who.sendDisconnect(who.user.id, who.strings.get(
								thisClass, "shuttinggoodbye"));
					} catch (IOException e) {
					}
				}
			}
		...
		// Print some stats
		serverOutput(strings.get(thisClass, "peakconn") + " " + peakConnections
				+ "\n");
		...
		if (!graphics) {
			System.out.println("");
			System.out.println(strings.get(thisClass, "shutdowncomplete"));
		}
	...
	private void usage() {
		...
		System.out.println("java babylonServer [" + portnumberParam
